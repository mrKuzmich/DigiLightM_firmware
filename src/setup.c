...
static void ir_paint(int32_t d) {
    ...
    if ((++div & 0x20) && ir_wait)
        lcd_puts_P("          ");
    else if ((ir_code == 0) || (ir_code == IR_NO_CMD)) {
        lcd_puts_P("--------  ");
    } else {
...
static void edit_input(int8_t d, uint16_t data) {
    change_val((uint8_t *) data, _IN_CNT, d);
    change_input();
}
...
static void edit_sensitivity(int8_t d, uint16_t data) {
    if (cfg.input != IN_OFF && cfg.input != IN_MIC)
        change_val(&cfg.sensitivity[cfg.input], MAX_LEVEL + 1, d);
}
...
static __flash const menu_t
ir_menu = _MENU(ir_menu_items);
...
__flash const menu_t
main_menu = _MENU(main_menu_items);
